...
def main(rank, args):
            ...
            for train_batch, (data, label) in enumerate(trainloader):
                ...
                if flag_batch:
                    if cnt == 0:
                        ...
                        cnt += 1
                        if cnt >= batch_large / batch_small:
                            ...
                            with torch.no_grad():
                                for idx, (param, param_prev) in enumerate(zip(module_dict[m], prev_module_dict[m])):
                                    if param.grad is None:
                                        ...
                                        g_est[m][idx] = dp.clone().detach()
                                        param.data.add_(dp, alpha=-lr)
                            train_cnt +=  1
                            m += 1
                            if m == len(module_dict):
                                m = 0
                            flag_batch = False
                            cnt = 0
                            flag_toss = True
    ...
    if rank == 0:
        ...
        write_res('./results/log_loss_CCD_' + str(args['lr']) + '_' + str(args['dataset']) + '_' + str(args['train_batch']) + '_new1.npy', log_loss_train)
        write_res('./results/log_acc_CCD_' + str(args['lr']) + '_' + str(args['dataset']) + '_' + str(args['train_batch']) + '_new1.npy', log_acc_test)
